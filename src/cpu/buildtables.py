...
ACCUM_HEADER = "static const uint8_t accumtable[256] = {"
...
#######################################################################################################################
###############################################  Output the accum table  ##############################################
#######################################################################################################################
def generateAccumTable(hFileName, header):
    spacing = 5
    hFileName.write("{}{}{}".format("\n", header, "\n"))
    hFileName.write(TABLE_MAP.format("", ""))
    for row in range(0, OPCODE_ROW_LEN):
        elements = ",".join(
            ["{0:>9}".format(("1" if opcodesList[opcodeItem][MODE_KEY_STR] == "acc" else "0"))[-spacing:] for opcodeItem in
             range(row * OPCODE_ROW_LEN, OPCODE_ROW_LEN * (row + 1))])
...
##################################################  Output a switch  ##################################################
#######################################################################################################################
def generateSwitch(hFileName):
    hFileName.write("\n#define OPCODE_SWITCH_TABLE \\\n")    
    hFileName.write("switch (opcode) { \\\n")    
    for op in range(0, 256):
        hFileName.write("    case {}: {}(); {}(); break; \\\n".format(op, opcodesList[op][MODE_KEY_STR], opcodesList[op][ACTN_KEY_STR]))
    hFileName.write("}\n")
...
#        generateTable(output_h_file, ACTN_CODE_HEADER, ACTN_KEY_STR)
        generateSwitch(output_h_file)
        generateAccumTable(output_h_file, ACCUM_HEADER)
